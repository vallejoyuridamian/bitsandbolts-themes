...
- Color preview, cancellation and commit present the same retained Themes recipe; consumers supply resolved color/state and share their abstract session owner. Never replace the native color input or maintain phase-specific swatch markup. Full and color-only Theme updates compose one retained color presenter. Identity variables, direct semantic pairs and foreground tokens share theme-roles.js; consumers do not duplicate these mappings. Drafts do not refresh fonts or unrelated modes.
